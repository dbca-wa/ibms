...
from sfm.models import CostCentre, Quarter, SFMMetric
from sfm.views import MeasurementValueJSON, OutputEntry, OutputReport, OutputUpload
...
urlpatterns = [
    ...
    path(
        "ajax/outcome/financial-year/",
        IbmsModelFieldJSON.as_view(model=Quarter, fieldname="quarter", return_pk=True),
        name="ajax_outcome_financialyear",
    ),
    path(
        "ajax/sfmmetric/metricid/",
        IbmsModelFieldJSON.as_view(model=SFMMetric, fieldname="metricID", return_pk=True),
